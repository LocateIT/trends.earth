...
from LDMS.layers import add_layer, create_local_json_metadata
from LDMS.worker import AbstractWorker, StartWorker
from LDMS.gui.DlgCalculateTCData import Ui_DlgCalculateTCData
from LDMS.gui.DlgCalculateTCSummaryTable import Ui_DlgCalculateTCSummaryTable
    ...
    def calculate_on_GEE(self, method, biomass_data):
        ...
        if resp:
            mb.pushMessage(QtWidgets.QApplication.translate("LDMS", "Submitted"),
                           QtWidgets.QApplication.translate("LDMS", "Total carbon submitted to Google Earth Engine."),
                           level=0, duration=5)
        else:
            mb.pushMessage(QtWidgets.QApplication.translate("LDMS", "Error"),
                           QtWidgets.QApplication.translate("LDMS", "Unable to submit total carbon task to Google Earth Engine."),
                           level=0, duration=5)
    ...
    def finished(self, result):
        if self.isCanceled():
            ...
            QtWidgets.QMessageBox.information(None, QtWidgets.QApplication.translate("LDMS", "Success"),
                                          QtWidgets.QApplication.translate("LDMS", u'Summary table saved to {}'.format(self.outout_file)))
        else:
            QtWidgets.QMessageBox.critical(None, QtWidgets.QApplication.translate("LDMS", "Error"),
                                       QtWidgets.QApplication.translate("LDMS", u"Error saving output table - check that {} is accessible and not already open.".format(self.output_file)))
...
class DlgCalculateTCSummaryTable(DlgCalculateBase, Ui_DlgCalculateTCSummaryTable):
    ...
    def select_output_file_table(self):
        filename = QSettings().value("LDMS/output_filename_{}".format(__name__), None)
        folder = QSettings().value("LDMS/output_dir", None)
        if filename:
        ...
        if f:
            if os.access(os.path.dirname(f), os.W_OK):
                ...
                QSettings().setValue("LDMS/output_filename_{}".format(__name__), f)
