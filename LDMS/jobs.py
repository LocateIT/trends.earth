...
from LDMS.gui.DlgJobs import Ui_DlgJobs
from LDMS.gui.DlgJobsDetails import Ui_DlgJobsDetails
from LDMS.plot import DlgPlotTimeries

from LDMS import log
from LDMS.api import get_user_email, get_execution
from LDMS.download import Download, check_hash_against_etag, DownloadError
from LDMS.layers import add_layer
from LDMS.schemas.schemas import LocalRaster, LocalRasterSchema
...
class DlgJobs(QtWidgets.QDialog, Ui_DlgJobs):
    ...
    def __init__(self, parent=None):
        ...
        jobs_cache = self.settings.value("LDMS/jobs_cache", None)
        if jobs_cache:
    ...
    def btn_refresh(self):
        ...
        if email:
            ...
            if jobs:
                ...
                self.settings.setValue("LDMS/jobs_cache", self.jobs)
            ...
            if job['results'].get('type') != 'TimeSeriesTable':
                ...
                while not f:
                    ...
                    f, _ = QtWidgets.QFileDialog.getSaveFileName(self,
                                                          ...
                                                          self.settings.value("LDMS/output_dir", None),
                    ...
                    if f:
                        if os.access(os.path.dirname(f), os.W_OK):
                            self.settings.setValue("LDMS/output_dir", os.path.dirname(f))
