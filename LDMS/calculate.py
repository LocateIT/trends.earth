...
from LDMS import log, GetTempFilename
from LDMS.api import run_script
# from LDMS.timeseries import DlgTimeseries
from LDMS.gui.DlgCalculate import Ui_DlgCalculate
from LDMS.gui.DlgCalculateLD import Ui_DlgCalculateLD
from LDMS.gui.DlgCalculateTC import Ui_DlgCalculateTC
from LDMS.gui.DlgCalculateRestBiomass import Ui_DlgCalculateRestBiomass
from LDMS.gui.DlgCalculateUrban import Ui_DlgCalculateUrban
from LDMS.gui.DlgCalculateMedalus import Ui_DlgCalculateMedalus
from LDMS.gui.DlgCalculateForest import Ui_DlgCalculateForest
from LDMS.gui.DlgTimeseries import Ui_DlgTimeseries
from LDMS.gui.DlgCalculateForestFire import Ui_DlgCalculateForestFire

from LDMS.gui.WidgetSelectArea import Ui_WidgetSelectArea
from LDMS.gui.WidgetCalculationOptions import Ui_WidgetCalculationOptions
from LDMS.gui.WidgetCalculationOutput import Ui_WidgetCalculationOutput
from LDMS.download import read_json, get_admin_bounds, get_cities
from LDMS.worker import AbstractWorker

mb = iface.messageBar()


if QSettings().value("LDMS/binaries_enabled", False) == 'True':
    try:
        from trends_earth_binaries.calculate_numba import *
        log("Using numba-compiled version of calculate_numba.")
    except (ModuleNotFoundError, ImportError) as e:
        from LDMS.calculate_numba import *
...
class DlgCalculateLD(QtWidgets.QDialog, Ui_DlgCalculateLD):
    def __init__(self, parent=None):
        ...
        from LDMS.calculate_prod import DlgCalculateProd
        from LDMS.calculate_lc import DlgCalculateLC
        from LDMS.calculate_soc import DlgCalculateSOC
        from LDMS.calculate_ldn import DlgCalculateOneStep, DlgCalculateLDNSummaryTableAdmin
...
class DlgCalculateTC(QtWidgets.QDialog, Ui_DlgCalculateTC):
    def __init__(self, parent=None):
        ...
        # TODO: Bad style - fix when refactoring
        from LDMS.calculate_tc import DlgCalculateTCData
        from LDMS.calculate_tc import DlgCalculateTCSummaryTable
...
class DlgCalculateRestBiomass(QtWidgets.QDialog, Ui_DlgCalculateRestBiomass):
    def __init__(self, parent=None):
        ...
        # TODO: Bad style - fix when refactoring
        from LDMS.calculate_rest_biomass import DlgCalculateRestBiomassData
        from LDMS.calculate_rest_biomass import DlgCalculateRestBiomassSummaryTable
...
class DlgCalculateForest(QtWidgets.QDialog, Ui_DlgCalculateForest):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        from LDMS.calculate_ff import DlgCalculateForestFire
...
class DlgCalculateMedalus(QtWidgets.QDialog, Ui_DlgCalculateMedalus):
    def __init__(self, parent=None):
        ...
        # TODO: Bad style - fix when refactoring
        from LDMS.calculate_cqi import DlgCalculateCQI
        # from LDMS.calculate_aqi import DlgCalculateAQI
        from LDMS.calculate_sqi import DlgCalculateSQI
        # from LDMS.calculate_vqi import DlgCalculateVQI
        ...
        self.btn_calculate_aqi.clicked.connect(self.btn_calculate_aqi_clicked)
...
#         # TODO: Bad style - fix when refactoring
#         from LDMS.calculate_urban import DlgCalculateUrbanData
#         from LDMS.calculate_urban import DlgCalculateUrbanSummaryTable
...
class CalculationOptionsWidget(QtWidgets.QWidget, Ui_WidgetCalculationOptions):
    ...
    def showEvent(self, event):
        ...
        local_data_folder = QSettings().value("LDMS/localdata_dir", None)
        ...
        self.task_name.setText(QSettings().value("LDMS/CalculationOptionsWidget/task_name", None))
        self.task_notes.setHtml(QSettings().value("LDMS/CalculationOptionsWidget/task_notes", None))

    def task_name_changed(self, value=None):
        QSettings().setValue("LDMS/CalculationOptionsWidget/task_name", value)

    def task_notes_changed(self):
        QSettings().setValue("LDMS/CalculationOptionsWidget/task_notes", self.task_notes.toHtml())
    ...
    def open_folder_browse(self):
        ...
        folder = QtWidgets.QFileDialog.getExistingDirectory(self,
                                                        ...
                                                        QSettings().value("LDMS/localdata_dir", None))
        if folder:
            if os.access(folder, os.R_OK):
                QSettings().setValue("LDMS/localdata_dir", os.path.dirname(folder))
...
class CalculationOutputWidget(QtWidgets.QWidget, Ui_WidgetCalculationOutput):
    ...
    def select_output_basename(self):
        local_name = QSettings().value("LDMS/output_basename_{}".format(self.subclass_name), None)
        if local_name:
            initial_path = local_name
        else:
            initial_path = QSettings().value("LDMS/output_dir", None)
        ...
        if f:
            if os.access(os.path.dirname(f), os.W_OK):
                QSettings().setValue("LDMS/output_dir", os.path.dirname(f))
                QSettings().setValue("LDMS/output_basename_{}".format(self.subclass_name), f)
...
class AreaWidget(QtWidgets.QWidget, Ui_WidgetSelectArea):
    def __init__(self, parent=None):
        ...
        icon = QIcon(QPixmap(':/plugins/LDMS/icons/map-marker.svg'))
    ...
    def showEvent(self, event):
        ...
        buffer_checked = QSettings().value("LDMS/AreaWidget/buffer_checked", False) == 'True'
        area_from_option = QSettings().value("LDMS/AreaWidget/area_from_option", None)
        ...
        self.area_frompoint_point_x.setText(QSettings().value("LDMS/AreaWidget/area_frompoint_point_x", None))
        self.area_frompoint_point_y.setText(QSettings().value("LDMS/AreaWidget/area_frompoint_point_y", None))
        self.area_fromfile_file.setText(QSettings().value("LDMS/AreaWidget/area_fromfile_file", None))
        self.area_type_toggle(False)

        admin_0 = QSettings().value("LDMS/AreaWidget/area_admin_0", None)
        if admin_0:
            self.area_admin_0.setCurrentIndex(self.area_admin_0.findText(admin_0))
            self.populate_admin_1()

        area_from_option_secondLevel = QSettings().value("LDMS/AreaWidget/area_from_option_secondLevel", None)
        ...
        secondLevel_area_admin_1 = QSettings().value("LDMS/AreaWidget/secondLevel_area_admin_1", None)
        ...
        secondLevel_city = QSettings().value("LDMS/AreaWidget/secondLevel_city", None)
        if secondLevel_city:
            self.populate_cities()
            self.secondLevel_city.setCurrentIndex(self.secondLevel_city.findText(secondLevel_city))

        buffer_size = QSettings().value("LDMS/AreaWidget/buffer_size", None)
        if buffer_size:
            self.buffer_size_km.setValue(int(buffer_size))
        self.groupBox_buffer.setChecked(buffer_checked)

    def admin_0_changed(self):
        QSettings().setValue("LDMS/AreaWidget/area_admin_0", self.area_admin_0.currentText())

    def admin_1_changed(self):
        QSettings().setValue("LDMS/AreaWidget/secondLevel_area_admin_1", self.secondLevel_area_admin_1.currentText())
        
    def admin_city_changed(self):
        QSettings().setValue("LDMS/AreaWidget/secondLevel_city", self.secondLevel_city.currentText())

    def file_changed(self):
        QSettings().setValue("LDMS/AreaWidget/area_fromfile_file", self.area_fromfile_file.text())

    def point_x_changed(self):
        QSettings().setValue("LDMS/AreaWidget/area_frompoint_point_x", self.area_frompoint_point_x.text())

    def point_y_changed(self):
        QSettings().setValue("LDMS/AreaWidget/area_frompoint_point_y", self.area_frompoint_point_y.text())

    def buffer_changed(self):
        QSettings().setValue("LDMS/AreaWidget/buffer_checked", str(self.groupBox_buffer.isChecked()))
        QSettings().setValue("LDMS/AreaWidget/buffer_size", self.buffer_size_km.value())
    ...
    def area_type_toggle(self, save=True):
        if self.area_frompoint.isChecked():
            if save: QSettings().setValue("LDMS/AreaWidget/area_from_option", 'point')
        ...
        if self.area_fromadmin.isChecked():
            if save: QSettings().setValue("LDMS/AreaWidget/area_from_option", 'admin')
        ...
        if self.area_fromfile.isChecked():
            if save: QSettings().setValue("LDMS/AreaWidget/area_from_option", 'file')
    ...
    def radioButton_secondLevel_toggle(self, save=True):
        if self.radioButton_secondLevel_region.isChecked():
            if save: QSettings().setValue("LDMS/AreaWidget/area_from_option_secondLevel", 'admin')
            self.secondLevel_area_admin_1.setEnabled(True)
            self.secondLevel_city.setEnabled(False)
        else:
            if save: QSettings().setValue("LDMS/AreaWidget/area_from_option_secondLevel", 'city')
    ...
    def open_vector_browse(self):
        initial_path = QSettings().value("LDMS/input_shapefile", None)
        if not initial_path:
            initial_path = QSettings().value("LDMS/input_shapefile_dir", None)
        if not initial_path:
        ...
        if vector_file:
            if os.access(vector_file, os.R_OK):
                QSettings().setValue("LDMS/input_shapefile", vector_file)
                QSettings().setValue("LDMS/input_shapefile_dir", os.path.dirname(vector_file))
...
class DlgCalculateBase(QtWidgets.QDialog):
    ...
    def showEvent(self, event):
        ...
        if self._has_output:
            f = QSettings().value("LDMS/output_basename_{}".format(self.get_subclass_name()), None)
            if f:
