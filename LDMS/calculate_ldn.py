...
from LDMS.calculate import DlgCalculateBase, get_script_slug, MaskWorker, \
    json_geom_to_geojson
from LDMS.lc_setup import lc_setup_widget, lc_define_deg_widget
from LDMS.layers import add_layer, create_local_json_metadata, get_band_infos, \
    delete_layer_by_filename
from LDMS.schemas.schemas import BandInfo, BandInfoSchema
from LDMS.gui.DlgCalculateOneStep import Ui_DlgCalculateOneStep
from LDMS.gui.DlgCalculateLDNSummaryTableAdmin import Ui_DlgCalculateLDNSummaryTableAdmin
from LDMS.worker import AbstractWorker, StartWorker
from LDMS.summary import *
from LDMS.summary_numba import merge_xtabs, xtab

from LDMS.calculate_numba import ldn_make_prod5, ldn_recode_state, \
...
class DlgCalculateOneStep(DlgCalculateBase, Ui_DlgCalculateOneStep):
    ...
    def showEvent(self, event):
        ...
        # from LDMS.download import read_json
        # from LDMS.worker import AbstractWorker, StartWorker
    ...
    def btn_calculate(self):
        ...
        if (self.year_final.date().year() - self.year_initial.date().year()) < 10:
            QtWidgets.QMessageBox.critical(None, QtWidgets.QApplication.translate("LDMS", "Error"),
                                       QtWidgets.QApplication.translate("LDMS", "Initial and final year must be at least 10 years apart."))
            return
        ...
        if resp:
            mb.pushMessage(QtWidgets.QApplication.translate("LDMS", "Submitted"),
                           QtWidgets.QApplication.translate("LDMS", "SDG sub-indicator task submitted to Google Earth Engine."),
                           level=0, duration=5)
        else:
            mb.pushMessage(QtWidgets.QApplication.translate("LDMS", "Error"),
                           QtWidgets.QApplication.translate("LDMS", "Unable to submit SDG sub-indicator task to Google Earth Engine."),
                           level=0, duration=5)
...
class DlgCalculateLDNSummaryTableAdmin(DlgCalculateBase, Ui_DlgCalculateLDNSummaryTableAdmin):
    ...
    def select_output_file_layer(self):
        f, _ = QtWidgets.QFileDialog.getSaveFileName(self,
                                              ...
                                              QSettings().value("LDMS/output_dir", None),
...
def make_summary_table(soc_totals, lc_totals, trans_prod_xtab, sdg_tbl_overall, 
                       ...
                       soc_years, out_file):
    def tr(s):
        return QtWidgets.QApplication.translate("LDMS", s)
    ...
    try:
        ...
        QtWidgets.QMessageBox.information(None, QtWidgets.QApplication.translate("LDMS", "Success"),
                                      QtWidgets.QApplication.translate("LDMS", u'Indicator table saved to {}'.format(out_file)))

    except IOError:
        log(u'Error saving {}'.format(out_file))
        QtWidgets.QMessageBox.critical(None, QtWidgets.QApplication.translate("LDMS", "Error"),
                                   QtWidgets.QApplication.translate("LDMS", u"Error saving output table - check that {} is accessible and not already open.".format(out_file)))
