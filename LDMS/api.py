...
from LDMS.worker import AbstractWorker, start_worker
...
def get_user_email(warn=True):
    email = QtCore.QSettings().value("LDMS/email", None)
    if warn and email is None:
        QtWidgets.QMessageBox.critical(None,
                                   QtWidgets.QApplication.translate("LDMS", "Error"),
                                   QtWidgets.QApplication.translate("LDMS", "Please register with LDMS before using this function."))
        return None
    else:
        return email
...
class Request(object):
    ...
    def start(self):
        try:
            ...
            start_worker(worker, iface, QtWidgets.QApplication.translate("LDMS", u'Contacting {} server...'.format(self.server_name)))
            pause.exec_()
            ...
            QtWidgets.QMessageBox.critical(None,
                                       QtWidgets.QApplication.translate("LDMS", "Error"),
                                       QtWidgets.QApplication.translate("LDMS", u"Unable to login to {} server. Check your internet connection.".format(self.server_name)))
            resp = None
        except requests.exceptions.Timeout:
            log('API unable to login - general error')
            QtWidgets.QMessageBox.critical(None,
                                       QtWidgets.QApplication.translate("LDMS", "Error"),
                                       QtWidgets.QApplication.translate("LDMS", u"Unable to connect to {} server.".format(self.server_name)))
            resp = None
...
def login(email=None, password=None):
    ...
    if (password == None):
        password = QtCore.QSettings().value("LDMS/password", None)
    if not email or not password:
        log('API unable to login - check username/password')
        QtWidgets.QMessageBox.critical(None,
                                   QtWidgets.QApplication.translate("LDMS", "Error"),
                                   QtWidgets.QApplication.translate("LDMS", "Unable to login to LDMS. Check your username and password."))
        return None

    resp = call_api('/auth', method='post', payload={"email": email, "password": password})

    if resp != None:
        QtCore.QSettings().setValue("LDMS/email", email)
        QtCore.QSettings().setValue("LDMS/password", password)
