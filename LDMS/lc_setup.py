...
from LDMS.gui.DlgCalculateLCSetAggregation import Ui_DlgCalculateLCSetAggregation
from LDMS.gui.WidgetLCDefineDegradation import Ui_WidgetLCDefineDegradation
from LDMS.gui.WidgetLCSetup import Ui_WidgetLCSetup
...
class DlgCalculateLCSetAggregation(QtWidgets.QDialog, Ui_DlgCalculateLCSetAggregation):
    ...
    def btn_load_pressed(self):
        f, _ = QtWidgets.QFileDialog.getOpenFileName(self,
                                              ...
                                              QSettings().value("LDMS/lc_def_dir", None),
                                              self.tr('Land cover definition (*.json)'))
        if f:
            if os.access(f, os.R_OK):
                QSettings().setValue("LDMS/lc_def_dir", os.path.dirname(f))
            else:
