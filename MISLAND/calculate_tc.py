...
from MISLAND.layers import add_layer, create_local_json_metadata
from MISLAND.worker import AbstractWorker, StartWorker
from MISLAND.gui.DlgCalculateTCData import Ui_DlgCalculateTCData
from MISLAND.gui.DlgCalculateTCSummaryTable import Ui_DlgCalculateTCSummaryTable
    ...
    def calculate_on_GEE(self, method, biomass_data):
        ...
        if resp:
            mb.pushMessage(QtWidgets.QApplication.translate("MISLAND", "Submitted"),
                           QtWidgets.QApplication.translate("MISLAND", "Total carbon submitted to Google Earth Engine."),
                           level=0, duration=5)
        else:
            mb.pushMessage(QtWidgets.QApplication.translate("MISLAND", "Error"),
                           QtWidgets.QApplication.translate("MISLAND", "Unable to submit total carbon task to Google Earth Engine."),
                           level=0, duration=5)
...
def write_excel_summary(forest_loss, carbon_loss, area_missing, area_water, 
    ...
    def tr(s):
        return QtWidgets.QApplication.translate("MISLAND", s)
...
class SummaryTask(QgsTask):
    ...
    def finished(self, result):
        if self.isCanceled():
            ...
            QtWidgets.QMessageBox.information(None, QtWidgets.QApplication.translate("MISLAND", "Success"),
                                          QtWidgets.QApplication.translate("MISLAND", u'Summary table saved to {}'.format(self.outout_file)))
        else:
            QtWidgets.QMessageBox.critical(None, QtWidgets.QApplication.translate("MISLAND", "Error"),
                                       QtWidgets.QApplication.translate("MISLAND", u"Error saving output table - check that {} is accessible and not already open.".format(self.output_file)))
...
class DlgCalculateTCSummaryTable(DlgCalculateBase, Ui_DlgCalculateTCSummaryTable):
    ...
    def select_output_file_table(self):
        filename = QSettings().value("MISLAND/output_filename_{}".format(__name__), None)
        folder = QSettings().value("MISLAND/output_dir", None)
        if filename:
        ...
        if f:
            if os.access(os.path.dirname(f), os.W_OK):
                ...
                QSettings().setValue("MISLAND/output_filename_{}".format(__name__), f)
