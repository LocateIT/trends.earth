...
from MISLAND.gui.DlgCalculateOneStep import Ui_DlgCalculateOneStep
from MISLAND.gui.DlgCalculateLDNSummaryTableAdmin import Ui_DlgCalculateLDNSummaryTableAdmin
    ...
    def btn_calculate(self):
        ...
        if (self.year_final.date().year() - self.year_initial.date().year()) < 10:
            QtWidgets.QMessageBox.critical(None, QtWidgets.QApplication.translate("MISLAND", "Error"),
                                       QtWidgets.QApplication.translate("MISLAND", "Initial and final year must be at least 10 years apart."))
            return
        ...
        if resp:
            mb.pushMessage(QtWidgets.QApplication.translate("MISLAND", "Submitted"),
                           QtWidgets.QApplication.translate("MISLAND", "SDG sub-indicator task submitted to Google Earth Engine."),
                           level=0, duration=5)
        else:
            mb.pushMessage(QtWidgets.QApplication.translate("MISLAND", "Error"),
                           QtWidgets.QApplication.translate("MISLAND", "Unable to submit SDG sub-indicator task to Google Earth Engine."),
                           level=0, duration=5)
...
def make_summary_table(soc_totals, lc_totals, trans_prod_xtab, sdg_tbl_overall, 
    ...
    try:
        ...
        QtWidgets.QMessageBox.information(None, QtWidgets.QApplication.translate("MISLAND", "Success"),
                                      QtWidgets.QApplication.translate("MISLAND", u'Indicator table saved to {}'.format(out_file)))

    except IOError:
        log(u'Error saving {}'.format(out_file))
        QtWidgets.QMessageBox.critical(None, QtWidgets.QApplication.translate("MISLAND", "Error"),
                                   QtWidgets.QApplication.translate("MISLAND", u"Error saving output table - check that {} is accessible and not already open.".format(out_file)))
