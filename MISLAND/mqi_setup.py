...
from MISLAND.gui.WidgetMQISetup import Ui_WidgetMQISetup
...
class DlgCalculateSetAggregation(QtWidgets.QDialog, Ui_DlgCalculateLCSetAggregation):
    ...
    def btn_save_pressed(self):
        f, _ = QtWidgets.QFileDialog.getSaveFileName(self,
                                              QtWidgets.QApplication.translate('DlgCalculateSetAggregation',
                                                                           'Choose where to save this soil texture definition'),
                                              QSettings().value("MISLAND/st_def_dir", None),
                                              QtWidgets.QApplication.translate('DlgCalculateSetAggregation',
                                                                           'Soil Texture definition (*.json)'))
        if f:
...
class MQISetupWidget(QtWidgets.QWidget, Ui_WidgetMQISetup):
    def __init__(self, parent=None):
        super(MQISetupWidget, self).__init__(parent)

        self.setupUi(self)

        default_lu_class_file = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                         'data', 'land_cover_classes_ESA_to_lu.json')
        self.dlg_lu_agg = DlgCalculateSetAggregation(read_class_file(default_lu_class_file),{'No data': -32768,
                              '1.0': 10,
                              '1.1': 11,
                              '1.2': 12,
                              '1.3': 13,
        ...
        self.use_lu_agg_edit.clicked.connect(self.lu_agg_custom_edit)

    def lu_agg_custom_edit(self):
        self.dlg_lu_agg.exec_()


mqi_setup_widget = MQISetupWidget()