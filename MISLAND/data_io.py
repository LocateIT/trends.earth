...
from MISLAND.gui.DlgDataIO import Ui_DlgDataIO
from MISLAND.gui.DlgDataIOLoadTE import Ui_DlgDataIOLoadTE
from MISLAND.gui.DlgDataIOLoadTESingleLayer import Ui_DlgDataIOLoadTESingleLayer
...
from MISLAND.gui.DlgDataIOImportMedalus import Ui_DlgDataIOImportMedalus
...
from MISLAND.gui.WidgetDataIOImportSelectRasterOutput import Ui_WidgetDataIOImportSelectRasterOutput
from MISLAND.gui.WidgetDataIOSelectTELayerExisting import Ui_WidgetDataIOSelectTELayerExisting
from MISLAND.gui.WidgetDataIOSelectTELayerImport import Ui_WidgetDataIOSelectTELayerImport
...
class DlgDataIO(QtWidgets.QDialog, Ui_DlgDataIO):
    def __init__(self, parent=None):
        ...
        self.dlg_DataIOLoad_drain =  DlgDataIOImportSQI('Drainage (3 class)', {'No data': -32768,
                        'Well Drained': 1.0,
        ...
        self.dlg_DataIOLoad_pm = DlgDataIOImportSQI('Parent material (3 class)', {'No data': -32768,
                        'Good': 1.0,
                        'Moderate': 1.7,
                        'Poor': 2.0,
                        })
        self.dlg_DataIOLoad_rock = DlgDataIOImportSQI('Rock fragments (3 class)', {'No data': -32768,
                        'Very Stony': 1,
                        'Stony': 1.3,
                        'Bare to slightly stony': 2,
                        })
        self.dlg_DataIOLoad_texture = DlgDataIOImportSQI('Soil texture (4 class)', {'No data': -32768,
                        'Good': 1,
                        'Moderate': 1.2,
                        'Poor': 1.6,
                        'Very Poor':2
        ...
        self.btn_drain.clicked.connect(self.run_drain)
        self.btn_texture.clicked.connect(self.run_texture)
    ...
    def run_drain(self):
        self.close()
        self.dlg_DataIOLoad_drain.exec_()

    def run_pm(self):
        self.close()
        self.dlg_DataIOLoad_pm.exec_()

    def run_texture(self):
        self.close()
        self.dlg_DataIOLoad_texture.exec_()

    def run_rock(self):
        self.close()
        self.dlg_DataIOLoad_rock.exec_()
...
class DlgDataIOLoadTE(DlgDataIOLoadTEBase, Ui_DlgDataIOLoadTE):
    ...
    def browse_file(self):
        f, _ = QtWidgets.QFileDialog.getOpenFileName(self,
                                              self.tr('Select a MISLAND output file'),
                                              QSettings().value("MISLAND/output_dir", None),
                                              self.tr('MISLAND metadata file (*.json)'))
    ...
    def update_layer_list(self, f):
        if f:
            ...
            if self.layer_list:
                ...
                self.layers_model.setStringList([])
                QtWidgets.QMessageBox.critical(None, self.tr("Error"),
                                           self.tr(u"{} does not appear to be a MISLAND output file".format(f)))
                self.layers_model.setStringList([])
                self.btn_view_metadata.setEnabled(False)
                return None
        else:
...
class ImportSelectFileInputWidget(QtWidgets.QWidget, Ui_WidgetDataIOImportSelectFileInput):
    ...
    def open_raster_browse(self):
        if self.lineEdit_raster_file.text():
            ...
            initial_file = QSettings().value("MISLAND/input_dir", None)
    ...
    def update_raster_layer(self, raster_file):
        ...
        if not os.access(raster_file, os.R_OK or not l.isValid()):
            ...
            QSettings().setValue("MISLAND/input_dir", os.path.dirname(raster_file))
    ...
    def update_vector_layer(self, vector_file):
        ...
        if not os.access(vector_file, os.R_OK) or not l.isValid():
            ...
            QSettings().setValue("MISLAND/input_dir", os.path.dirname(vector_file))
...
class ImportSelectRasterOutput(QtWidgets.QWidget, Ui_WidgetDataIOImportSelectRasterOutput):
    ...
    def save_raster(self):
        if self.lineEdit_output_file.text():
            ...
            initial_file = QSettings().value("MISLAND/output_dir", None)
